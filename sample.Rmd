...
As we observe the histogram, with a mean of 4.9900252 , we can compare to 1/lambda (5), we observed the distribution of means is centered in the theoretical mean.
...
plot1 <- plot1 +geom_histogram(aes(y=..density..), colour="black",fill="green")
plot1<-plot1+labs(title="Distribution of Means of rexp", y="Density")
plot1<-plot1 +stat_function(fun=dnorm,args=list( mean=1/lambda, sd=sqrt(varxp)),color = "orange", size = 1.0)
plot1<-plot1 +stat_function(fun=dnorm,args=list( mean=mean(mns), sd=sqrt(varmean)),color = "black", size = 1.0)
print(plot1)
```
